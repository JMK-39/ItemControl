...
public final class ItemSearchCache {
    ...
    public static Set<String> getRegistryTagIdsForStack(ItemStack stack) {
        ...
        ItemBanControl.withSkip(() -> {
            stack.getTags().forEach(tagKey -> {
                ...
                if (!location.getNamespace().equals("itemcontrol")) {
    ...
    public static Set<String> getEffectiveTagIdsForId(String idStr) {
        TreeSet<String> result = new TreeSet<>(getRegistryTagIdsForId(idStr));
        result.addAll(BanItemConfig.getEffectiveExtraTagIds(idStr));
        if (result.isEmpty()) return Collections.emptySet();
        return Collections.unmodifiableSet(result);
    }
    ...
    private static Snapshot buildIfNeeded() {
        long now = System.currentTimeMillis();
        boolean sharedIndexReady = KineticItemSearch.ready();
        Snapshot current = snapshot;
        if (current.isAlive(now, sharedIndexReady)) return current;

        synchronized (SNAPSHOT_LOCK) {
            sharedIndexReady = KineticItemSearch.ready();
            current = snapshot;
            if (current.isAlive(now, sharedIndexReady)) return current;
            ...
            for (Item item : KineticRegistries.items().values()) {
                ...
                if (rl != null && !rl.getNamespace().equals("itemcontrol")) {
            ...
            KineticRegistries.items().tagIds().forEach(loc -> {
                if (!loc.getNamespace().equals("itemcontrol")) {
    ...
    private static List<KineticItemSearch.CachedItem> buildRawItemSnapshot() {
        ...
        if (cached != null && !cached.isEmpty()) {
            ...
            for (KineticItemSearch.CachedItem item : cached) {
                if (item == null || BanItemConfig.VOID_ID.equals(item.id())) continue;
                result.add(item);
            }
            return result;
        }
    ...
    private record Snapshot(
            List<KineticItemSearch.CachedItem> allItems,
            Map<String, String> searchDataById,
            Map<String, Set<String>> tagsById,
            List<String> allMods,
            List<String> allTags,
            int allItemsHash,
            boolean sharedIndexReady,
            long createdAt
    ) {
        boolean isAlive(long now, boolean currentSharedIndexReady) {
            return !allItems.isEmpty()
                    && sharedIndexReady == currentSharedIndexReady
